...
class impscan(interfaces.plugins.PluginInterface, timeliner.TimeLinerInterface):
    ...
    def process_dump(
            ...
            proc: interfaces.objects.ObjectInterface) -> str:
        ...
        try:
            ...
            pex = pefile.PE(data=file_handle.getvalue(), fast_load=True)
            pex.parse_data_directories(directories=pefile.DIRECTORY_ENTRY["IMAGE_DIRECTORY_ENTRY_IMPORT"])
            if hasattr(pex, 'DIRECTORY_ENTRY_IMPORT'):
                result_text = "IMPORT: "+str([(imp.name.decode(errors='replace') if imp.name else 'None', entry.dll.decode(errors='replace') if entry.dll else 'None') for entry in pex.DIRECTORY_ENTRY_IMPORT for imp in entry.imports])
            pex.parse_data_directories(directories=pefile.DIRECTORY_ENTRY["IMAGE_DIRECTORY_ENTRY_EXPORT"])
            if hasattr(pex, 'DIRECTORY_ENTRY_EXPORT'):
                result_text += "EXPORT: "+str([(exp.name.decode(errors='replace') if exp.name else 'None') for exp in pe.DIRECTORY_ENTRY_EXPORT.symbols])
            file_handle.close()
